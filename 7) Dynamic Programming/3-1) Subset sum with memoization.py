...
class Solution:
    def solve(self, input_array, remaining_sum, n, matrix):
        ...
        # Returning from memoized matrix if available
        ...
        if input_array[n-1] <= remaining_sum:
            ...
            # Memoize current recursive answer
            matrix[n][remaining_sum] = self.solve(input_array, remaining_sum-input_array[n-1], n-1, matrix) or self.solve(input_array, remaining_sum, n-1, matrix)
            return matrix[n][remaining_sum]
        
        # Else we cannot include current item in remaining_sum
        # Memoize current recursive answer
        matrix[n][remaining_sum] = self.solve(input_array, remaining_sum, n-1, matrix)
        return matrix[n][remaining_sum]

    def subsetsum(self, input_array, sum):
        matrix = [[None for i in range(sum+1)] for j in range(len(input_array)+1)]
        return self.solve(input_array, sum, len(input_array), matrix)
