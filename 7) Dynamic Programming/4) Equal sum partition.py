...
class Solution:
    ...
    def canPartition(self, input_array):
        # Now for the equal sum partition
        # We sum all the input elements
        # if sum is even that means we can partition only if any set exists with the sum = total_sum/2
        # So if we have one set which sum is total_sum/2 that means another set also exists with the sum as total_sum/2, so we check that total_sum/2 sum is possible or not using subset sum problem
        # if sum is odd that means we cannot partition
        total_sum = sum(input_array)
        if total_sum%2 != 0:
            return False
        return self.subsetSum(input_array, total_sum//2)

sol = Solution()
input_array = [4, 3, 7]

print(sol.canPartition(input_array))